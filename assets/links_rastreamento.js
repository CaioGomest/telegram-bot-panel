(function ($) {
    ...
    function carregarLinks() {
        ...
        $.getJSON(urlApi + '?action=listar_links_rastreamento')
            .done(function (res) {
                if (!res.sucesso) {
                ...
                links.forEach(function (link) {
                    const urlGerada = escaparHtml(link.link_gerado || '');
                    $corpo.append(`
                        <tr>
                            ...
                            <td class="col-link" title="${urlGerada}">${urlGerada}</td>
                            <td>
                                <div class="col-acoes">
                                    <button class="btn-icon editar btn-editar-link" data-id="${escaparHtml(link.id)}" title="Editar">${icons.edit}</button>
                                    <button class="btn-icon btn-copiar-link" data-url="${urlGerada}" title="Copiar link">${icons.copy}</button>
    ...
    function carregarBots(botIdSelecionado) {
        ...
        $.getJSON(urlApi + '?action=listar_bots')
            .done(function (res) {
                ...
                bots.forEach(function (bot) {
                    const selecionado = botIdSelecionado && parseInt(bot.id) === parseInt(botIdSelecionado) ? 'selected' : '';
    ...
    function abrirModalEdicao(id) {
        $.getJSON(urlApi + '?action=obter_link_rastreamento&id=' + id)
            .done(function (res) {
    ...
    function salvarLink() {
        ...
        if (!botId) { exibirAviso('Selecione um bot.', 'erro'); return; }

        const action = modoEdicao ? 'editar_link_rastreamento' : 'criar_link_rastreamento';
        const payload = { titulo, identificador, bot_id: parseInt(botId) };
        if (modoEdicao) payload.id = parseInt(id);
        ...
        $.ajax({
            ...
            $btn.prop('disabled', false).text(modoEdicao ? 'Salvar alterações' : 'Gerar link');
        });
    }
    ...
    function excluirLink(id) {
        ...
        $.ajax({
            url: urlApi + '?action=excluir_link_rastreamento',
            method: 'POST',
            contentType: 'application/json',
