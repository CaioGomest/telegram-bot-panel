...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        _drawLink: function (linkId) {
            ...
            shape_path.setAttribute("class", "flowchart-link-path");
            group.appendChild(shape_path);
            linkData.internal.els.path = shape_path;

            var shape_path_anim = document.createElementNS("http://www.w3.org/2000/svg", "path");
            shape_path_anim.setAttribute("stroke-width", this.options.linkWidth.toString());
            shape_path_anim.setAttribute("fill", "none");
            shape_path_anim.setAttribute("class", "flowchart-link-animation");
            group.appendChild(shape_path_anim);
            linkData.internal.els.path_anim = shape_path_anim;
        ...
        _refreshLinkPositions: function (linkId) {
            ...
            if (this.options.verticalConnection) {
                ...
                if (linkData.internal.els.path_anim) {
                    linkData.internal.els.path_anim.setAttribute("d", 'M' + bezierFromX + ',' + (fromY) + ' C' + bezierFromX + ',' + (fromY + bezierIntensity) + ' ' + bezierToX + ',' + (toY - bezierIntensity) + ' ' + bezierToX + ',' + toY);
                }
                ...
                if (linkData.internal.els.path_anim) {
                    linkData.internal.els.path_anim.setAttribute("d", 'M' + bezierFromX + ',' + (fromY) + ' C' + (fromX + offsetFromX + distanceFromArrow + bezierIntensity) + ',' + fromY + ' ' + (toX - bezierIntensity) + ',' + toY + ' ' + bezierToX + ',' + toY);
                }
            ...
            if (this.options.canUserMoveOperators) {
                ...
                fullElement.operator.draggable({
                    ...
                    start: function (e, ui) {
                        ...
                        pointerX = (e.pageX - elementOffset.left) / self.positionRatio - parseInt($(this).css('left'), 10);
                        pointerY = (e.pageY - elementOffset.top) / self.positionRatio - parseInt($(this).css('top'), 10);
                    },
                    drag: function (e, ui) {
                        var elementOffset = self.element.offset();
                        var grid = self.options.grid;
                        
                        // Calcula posição corrigida pelo zoom
                        var newLeft = (e.pageX - elementOffset.left) / self.positionRatio - pointerX;
                        var newTop = (e.pageY - elementOffset.top) / self.positionRatio - pointerY;

                        // Aplica o grid se estiver configurado
                        if (grid) {
