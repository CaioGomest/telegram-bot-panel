...
$logFile = $logDir . '/webhook_efi.log';
...
function processar_bloco_local(string $token, $idChat, array $operador, int $idUsuarioDono): void {
    ...
    if ($tipo === 'message') {
        ...
        if ($caminho) {
            ...
            if ($real) {
                requisicao_telegram_local($token, 'sendPhoto', ['chat_id' => $real, 'photo' => $real]);
            }
        }
    } elseif ($tipo === 'botoes') {
...
function executar_fluxo_continuacao(string $token, string $idChat, int $botId, string $idOperadorInicial, int $idUsuarioDono, string $conector = 'output_pago'): void {
    global $pdo;
    ...
    if (!$proximoId) {
        logWebhook("Nenhuma conexão saindo de '$conector' no bloco $idOperadorInicial.");
        return;
    }

    logWebhook("Continuando fluxo ($conector) para chat $idChat a partir de $proximoId");
...
/**
 * Libera ou renova acesso ao grupo para uma venda paga.
 * Retorna o link de convite gerado ou null em caso de falha.
 */
function liberarAcessoGrupo(array $venda, string $tokenBot): ?string {
    global $pdo;
    ...
    // Busca expiração atual para extensão correta (renova a partir da data atual, não do NOW())
    $stmtMembro = $pdo->prepare("SELECT data_expiracao FROM membros_grupos WHERE id_telegram = ? AND id_grupo_telegram = ? AND bot_id = ?");
    $stmtMembro->execute([$venda['id_telegram'], $idGrupo, $venda['bot_id']]);
    $membroAtual = $stmtMembro->fetchColumn();

    if ($membroAtual && strtotime($membroAtual) > time()) {
        // Ainda tem acesso ativo: estende a partir da data de expiração atual
        $dataExpiracao = date('Y-m-d H:i:s', strtotime($membroAtual) + ($tempoMinutos * 60));
    } else {
        // Expirado ou novo: começa do agora
        $dataExpiracao = date('Y-m-d H:i:s', time() + ($tempoMinutos * 60));
    }

    // Revoga o link anterior do mesmo usuário/grupo para evitar reuso e compartilhamento.
    ...
    if ($linkAnterior !== '') {
        requisicao_telegram_local($tokenBot, 'revokeChatInviteLink', [
            'chat_id' => $idGrupo,
            'invite_link' => $linkAnterior
        ]);
    }

    $invite = requisicao_telegram_local($tokenBot, 'createChatInviteLink', [
        'chat_id' => $idGrupo,
        'member_limit' => 1,
...
/**
 * Remove membro do grupo por falta de pagamento.
 */
function removerMembroGrupo(string $idTelegram, string $idGrupo, string $token, int $membroId): void {
    global $pdo;

    requisicao_telegram_local($token, 'banChatMember', [
        'chat_id' => $idGrupo,
        'user_id' => $idTelegram,
        'until_date' => time() + 35
    ]);
    requisicao_telegram_local($token, 'unbanChatMember', [
        'chat_id' => $idGrupo,
        'user_id' => $idTelegram,
        'only_if_banned' => true
    ]);
...
if (empty($entrada)) {
    // Ping de validação de URL pela Efí
    logWebhook("Ping de validação recebido.");
    http_response_code(200);
    exit;
}

logWebhook("Payload recebido: " . $entrada);

$notificacao = json_decode($entrada, true);

if (!is_array($notificacao)) {
    logWebhook("Payload inválido (não é JSON).");
    http_response_code(200);
    exit;
}
...
if (isset($notificacao['cobsr'])) {
    foreach ($notificacao['cobsr'] as $cobsr) {
        ...
        if (empty($idRec)) {
            logWebhook("cobsr sem idRec. Ignorado.");
            continue;
        }

        logWebhook("PIX Automático | idRec=$idRec | status=$status");

        // Busca a venda original pelo id_assinatura (idRec)
        ...
        if (!$venda) {
            logWebhook("Nenhuma venda encontrada para idRec=$idRec.");
            continue;
        }
        ...
        if ($status === 'ATIVA') {
            // Cobrança automática bem-sucedida — estende acesso
            logWebhook("Cobrança recorrente PAGA para idRec=$idRec. Renovando acesso do usuário $idTelegram.");

            $link = liberarAcessoGrupo($venda, $tokenBot);
            ...
            requisicao_telegram_local($tokenBot, 'sendMessage', [
                'chat_id' => $idTelegram,
                'text' => $msgRenovacao,
                'parse_mode' => 'Markdown'
            ]);

            registrarAtividade($idDono, 'venda', 'Renovação Automática', "PIX Automático renovado para usuário $idTelegram (idRec=$idRec).");

        } elseif (in_array($status, ['REJEITADA', 'CANCELADA', 'EXPIRADA'])) {
            // Cobrança falhou ou recorrência cancelada — remove acesso
            logWebhook("Cobrança recorrente FALHOU ($status) para idRec=$idRec. Removendo usuário $idTelegram.");

            if ($membro && $membro['status'] === 'ativo') {
                removerMembroGrupo($idTelegram, $idGrupo, $tokenBot, (int)$membro['id']);
            }

            registrarAtividade($idDono, 'sistema', 'Remoção por Falha', "PIX Automático falhou ($status) para usuário $idTelegram (idRec=$idRec).");
        } else {
            logWebhook("Status '$status' para idRec=$idRec não requer ação imediata.");
        }
    }
...
// ── PIX comum (pago via webhook) — campo 'pix' ────────────────────────────────
if (!isset($notificacao['pix'])) {
    logWebhook("Payload sem 'pix' nem 'cobsr'. Ignorado.");
    http_response_code(200);
    exit;
}
...
foreach ($notificacao['pix'] as $pix) {
    ...
    if (empty($txid)) {
        logWebhook("PIX sem txid. Ignorado.");
        continue;
    }
    ...
    if (!$venda) {
        logWebhook("Venda não encontrada para TXID=$txid.");
        continue;
    }

    if ($venda['status'] === 'pago') {
        logWebhook("Venda #{$venda['id']} já paga. Ignorando duplicata.");
        continue;
    }
    ...
    try {
        ...
        $pdo->commit();
        logWebhook("Venda #{$venda['id']} marcada como PAGO.");
    } catch (Exception $e) {
        $pdo->rollBack();
        logWebhook("Erro ao atualizar venda #{$venda['id']}: " . $e->getMessage());
        continue;
    }
    ...
    if ($idDono) {
        ...
        registrarAtividade($idDono, 'venda', 'Venda Aprovada', "PIX R$ {$valorFmt} pago (TXID=$txid).");
    ...
    if (!$tokenBot) {
        logWebhook("Token do bot não encontrado para venda #{$venda['id']}.");
        continue;
    }
    ...
    if (!empty($venda['id_grupo_telegram'])) {
        ...
        if ($link) {
            ...
            logWebhook("Falha ao gerar link para venda #{$venda['id']}.");
        }
    }

    $respMsg = requisicao_telegram_local($tokenBot, 'sendMessage', [
        'chat_id'    => $venda['id_telegram'],
        'text'       => $msg,
        'parse_mode' => 'HTML'
    ]);
    logWebhook("Mensagem enviada: " . json_encode($respMsg));

    if (!empty($venda['id_operador_fluxo']) && $idDono) {
        executar_fluxo_continuacao($tokenBot, $venda['id_telegram'], $venda['bot_id'], $venda['id_operador_fluxo'], $idDono, 'output_pago');
    }
}
